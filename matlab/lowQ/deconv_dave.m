...
% initialization of constraints
 ...
 while iter<40
       ...
       %if (isempty(hL))
       %    hL=1; 
       %elseif(hL(1)~=1)   % DAB turn off forced h(1)=0
       %    hL=[1;hL] 
       %end
       nL=length(hL);
...
for ireal=1:nreal
    ...
    while iter<40
       ...
       drawnow
        ...
        hL=hL(~ismember(hL,hLrem));
        hL=union(hL,hLadd);
        %if (isempty(hL))   % DAB turn off setting h(1) = 0
        %    hL=1; 
        %elseif(hL(1)~=1)
        %    hL=[1;hL]; 
        %end
...
dx=24;              % m downstream of the upstream
v=dx/m_1;           % m/hr
...
tplot=linspace(0,4,500); dt_plot=tplot(2)-tplot(1);
...
txt ={['Kernel stats:'],['mass: ' num2str(m_0)],['mean (hr):   ' num2str(m_1)],['Var (hr^2): ' num2str(var)]};
text(0.25,20,txt)
