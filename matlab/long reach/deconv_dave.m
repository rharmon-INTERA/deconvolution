...
c=0.0*c;   % DAB's
c(1:n_corr_time)=[n_corr_time:-1:1]*dt*theta;  % DAB's
%pause
Q=toeplitz(c);
C=chol(Q);
iQ=inv(Q);
...
% initialization of constraints
 ...
 while iter<40
       ...
       hL=union(hL,hLadd);
       %if (isempty(hL))  % DAB Remove requirement that h(1)=0
       %    hL=1; 
       %elseif(hL(1)~=1)
       %    hL=[1;hL]; 
       %end
...
% mean_h=[mean(h_all(:,1:ireal),2); zeros(size(h)) ];
% h_fft=fft(mean_h);
% cov=(1/length(h))*ifft( h_fft.*conj(h_fft) );
% cov=cov(1:length(h));
% int_cov=dt*sum(cov);
% theta=cov(1)
...
trim=min(n_h,1200)
h_trim=h(1:trim);
m_time=dt*(0:length(h_trim)-1)';
m_0=dt*sum(h_trim)
m_1=(dt/m_0)*sum(m_time.*h_trim)
var=(dt/m_0)*sum(((m_time-m_1).^2).*h_trim) 
RMSE=sqrt(mean((J*h-y).^2))

%  Generate an Inverse Gaussian with same parameters

dx=200;              % m downstream of the upstream
v=dx/m_1           % m/hr
...
tplot=linspace(0,250,500); dtplot=tplot(2)-tplot(1);
...
txt ={['g(t) moments:'],['kernel mass: ' num2str(m_0)],['mean (hr):   ' num2str(m_1)],['Var (hr^2): ' num2str(var)]};
text(0.7,5,txt)
...
figure(22)
semilogy(t_h(1:end),h(1:end),'rx','MarkerSize',4);
xlabel('\tau [hr]');
ylabel('g [1/hr]');
hold on
%plot(t_h,mean(h_all(:,1:ireal),2),'r');
...
%txt ={['g(t) moments:'],['kernel mass: ' num2str(m_0)],['mean (hr):   ' num2str(m_1)],['Var (hr^2): ' num2str(var)]};
%text(0.7,5,txt)
%txt ={['ADE paramters:'],['v (m/hr): ' num2str(v)],['D (m^2/hr): ' num2str(Disp)]};
%text(0.7,7,txt)

hold off

%l_ut=(exp(beta*lambda*u-lambda*(tplot-u))).*
