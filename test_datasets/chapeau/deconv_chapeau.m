...
%%%%%%%%%%  Some reach details %%%%%%%%%%%%%%%%%%%%%
prefix='chapeau';
data = readtable('chapeau_data.csv');
time = data.time;
in = data.in;
out = data.out;
%%%%%%%%%%  Some user-defined numerical details %%%%%%%%%%%%%%%%%%%%%
% slope of the linear variogram
theta=2;       %  A first guess applied linearly to corr_time:
corr_time=9;   %  Only applies if less than the total h(t) length
% standard deviation of epistemic error (initial guess)
sigma = .1;
sigma_max= .15;   % DAB places a forced maximum on iterated sigma
...
%cov(1)=max(cov(1)-sigma^2, 0.01*cov(1));
...
%Some stats:
trim_time=min(dt*n_h,30);                  % DAB pick a trim time,    
